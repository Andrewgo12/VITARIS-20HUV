import { PatientData, ReferralData, VitalSigns, DocumentsData } from '@/context/FormContext';
...
export function validatePatientData(patient: PatientData): ValidationResult {
  ...
  if (!patient.identificationType) errors.push('Tipo de identificación es requerido');
  if (!patient.identificationNumber) errors.push('Número de identificación es requerido');
  ...
  if (!patient.affiliationRegime) errors.push('Régimen de afiliación es requerido');
  if (!patient.affiliateType) errors.push('Tipo de afiliado es requerido');
  if (!patient.affiliationNumber) errors.push('Número de afiliación es requerido');
  if (!patient.affiliationStatus) errors.push('Estado de afiliación es requerido');
  if (!patient.phone) errors.push('Teléfono es requerido');
  if (!patient.address) errors.push('Dirección es requerida');

  // Format validation
  if (patient.identificationNumber && !/^\d+$/.test(patient.identificationNumber)) {
...
export function validateReferralData(referral: ReferralData): ValidationResult {
  ...
  // Required fields validation
  if (!referral.referralService) errors.push('Servicio de remisión es requerido');
  if (!referral.referralReason) errors.push('Motivo de remisión es requerido');
  if (!referral.primaryDiagnosis) errors.push('Diagnóstico principal es requerido');
...
export function validateVitalSigns(vitals: VitalSigns): ValidationResult {
  ...
  if (!vitals.respiratoryRate) errors.push('Frecuencia respiratoria es requerida');
  ...
  if (!vitals.oxygenSaturation) errors.push('Saturación de oxígeno es requerida');
...
export function validateDocumentsData(documents: DocumentsData): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];

  // Required fields validation
  if (!documents.professionalName) errors.push('Nombre del profesional es requerido');
  if (!documents.professionalPosition) errors.push('Cargo del profesional es requerido');
  if (!documents.professionalPhone) errors.push('Teléfono del profesional es requerido');

  // Phone validation
  if (documents.professionalPhone && !/^\d{7,10}$/.test(documents.professionalPhone)) {
