...
import { Eye, CheckCircle, XCircle, AlertTriangle, FileText, LogOut } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 p-4">
      ...
      <div className="bg-white rounded-xl shadow p-6 mb-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Dashboard Médico - HUV</h1>
            ...
            <table className="w-full">
              <thead className="bg-slate-100 border-b">
                <tr>
                  ...
                  <th className="text-left p-4 font-semibold text-slate-700">Nombre Completo</th>
                  <th className="text-left p-4 font-semibold text-slate-700">EPS</th>
                  <th className="text-left p-4 font-semibold text-slate-700">Edad</th>
                  <th className="text-left p-4 font-semibold text-slate-700">Prioridad</th>
                  <th className="text-left p-4 font-semibold text-slate-700">Llegada</th>
              ...
              <tbody>
                {filteredPatients.map((patient, index) => (
                  <tr key={patient.id} className={`border-b hover:bg-slate-50 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}`}>
                    <td className="p-4">
                      <div className="font-mono text-sm">
                        <div className="font-medium">{patient.identificationType}</div>
                        <div className="text-slate-600">{patient.identificationNumber}</div>
                      </div>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      <div className="text-sm">
                        <div className="font-medium">{patient.arrivalTime.split(' ')[1]}</div>
                        <div className="text-slate-500">{patient.arrivalTime.split(' ')[0]}</div>
                      </div>
                    </td>
                    <td className="p-4">
                              ...
                              <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                  <div>
                                    ...
                                    <p className="font-mono">{selectedPatient.identificationType} {selectedPatient.identificationNumber}</p>
                                  </div>
                                  <div>
                                    <Label>Nombre</Label>
                                ...
                                <div>
                                  ...
                                  <p className="bg-slate-50 p-3 rounded">{selectedPatient.symptoms}</p>
                                </div>
                                <div className="grid grid-cols-3 gap-4">
                                  <div>
                                    <Label>Frecuencia Cardíaca</Label>
                                    <p className="font-mono">{selectedPatient.vitals.heartRate} lpm</p>
                                  </div>
                                  <div>
                                    <Label>Temperatura</Label>
                                    <p className="font-mono">{selectedPatient.vitals.temperature}°C</p>
                                  </div>
                                  <div>
                                    <Label>Presión Arterial</Label>
                                    <p className="font-mono">{selectedPatient.vitals.bloodPressure} mmHg</p>
                                  </div>
                                </div>
                              </div>
                            )}
                          ...
                          <DialogContent>
                            ...
                            <div className="space-y-4">
                              ...
                              <Select value={priorityRating} onValueChange={setPriorityRating}>
                                ...
                                <SelectContent>
                                  <SelectItem value="CRITICO">CRÍTICO - Requiere atención inmediata</SelectItem>
                                  <SelectItem value="SEVERO">SEVERO - Atención urgente</SelectItem>
                                  <SelectItem value="MODERADO">MODERADO - Puede esperar</SelectItem>
                                  <SelectItem value="LEVE">LEVE - No urgente</SelectItem>
                                </SelectContent>
                              </Select>
                              <Button 
                                onClick={() => {
                                  if (priorityRating) {
                                    handleSetPriority(patient.id, priorityRating);
                                    setPriorityRating('');
                                  }
                                }}
