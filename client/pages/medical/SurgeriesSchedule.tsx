  ...
  },
  {
    ...
    progress: {
      ...
      notes: [
        ...
        "Excelente visualización laparoscópica",
      ],
    },
    postOp: {
      recovery: "Excelente",
...
const mockOperatingRooms = [
  {
    ...
    status: "DISPONIBLE",
    type: "General",
    equipment: ["Laparoscopio", "Monitor"],
    capacity: 6,
  },
  {
    id: "Quirófano 2",
  ...
  },
  {
    ...
    type: "Ortopedia",
    equipment: ["C-arm", "Mesa ortopédica"],
    capacity: 8,
  },
  {
    id: "Quirófano 4",
    status: "DISPONIBLE",
    type: "Cardiovascular",
    equipment: ["Bypass", "Monitor avanzado"],
    capacity: 10,
  },
  {
    id: "Quirófano 5",
    status: "LIMPIEZA",
    type: "Neurocirugía",
    equipment: ["Microscopio", "Neuronavegador"],
    capacity: 8,
  },
];
...
export default function SurgeriesSchedule() {
  ...
  const filteredSurgeries = surgeries.filter((surgery) => {
    ...
    const matchesStatus =
      statusFilter === "all" || surgery.schedule.status === statusFilter;
    ...
    if (dateFilter === "today") {
      ...
      matchesDate =
        surgery.schedule.date === tomorrow.toISOString().split("T")[0];
    }
  ...
  const stats = {
    ...
    programadas: surgeries.filter((s) => s.schedule.status === "PROGRAMADA")
      .length,
    enCurso: surgeries.filter((s) => s.schedule.status === "EN_CURSO").length,
    completadas: surgeries.filter((s) => s.schedule.status === "COMPLETADA")
      .length,
    quirofanosDisponibles: operatingRooms.filter(
        ...
        <Tabs defaultValue="schedule" className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-1 text-sm">
                          <div>
                            <strong>Edad:</strong> {surgery.patient.age} años
                          </div>
                          <div>
                            <strong>Documento:</strong>{" "}
                            {surgery.patient.document}
                          </div>
                          <div>
                            <strong>Tipo sangre:</strong>{" "}
                            {surgery.patient.bloodType}
                          </div>
                          <div>
                            <strong>Peso:</strong> {surgery.patient.weight}
                          </div>
                          <div className="text-xs text-red-600">
                            <strong>Alergias:</strong>{" "}
                            {surgery.patient.allergies.join(", ")}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-1 text-sm">
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <span
                              className={`font-medium ${getUrgencyColor(surgery.surgery.urgency)}`}
                            >
                          ...
                          <div>
                            ...
                            {surgery.surgery.estimatedDuration}
                          </div>
                          <div>
                            <strong>Anestesia:</strong>{" "}
                            {surgery.surgery.anesthesiaType}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-1 text-sm">
                          ...
                          <div>
                            <strong>Fecha:</strong> {surgery.schedule.date}
                          </div>
                          <div>
                            <strong>Hora:</strong> {surgery.schedule.time} -{" "}
                            {surgery.schedule.endTime}
                          </div>
                          <div>
                            <strong>Quirófano:</strong> {surgery.schedule.room}
                          </div>
                          <div>
                            <strong>Pre-op:</strong>{" "}
                            {surgery.schedule.preOpTime}
                          </div>
                          <div>
                            <strong>Recuperación:</strong>{" "}
                            {surgery.schedule.postOpRoom}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-1 text-xs">
                          <div>
                            <strong>Cirujano:</strong> {surgery.team.surgeon}
                          </div>
                          <div>
                            <strong>Asistente:</strong> {surgery.team.assistant}
                          </div>
                          <div>
                            <strong>Anestesiólogo:</strong>{" "}
                            {surgery.team.anesthesiologist}
                          </div>
                          <div>
                            <strong>Enfermería:</strong> {surgery.team.nurse}
                          </div>
                          <div>
                            <strong>Técnico:</strong> {surgery.team.technician}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        {surgery.schedule.status === "EN_CURSO" && (
                          <div className="space-y-2">
                            <div className="text-sm">
                              ...
                              {surgery.progress.percentage}%
                            </div>
                            <Progress
                              value={surgery.progress.percentage}
                              className="h-2"
                            />
                        ...
                        <div className="space-y-2">
                          <Button
                            ...
                            className="w-full"
                            onClick={() =>
                              navigate(`/patient/${surgery.patient.id}`)
                            }
                          >
                            Ver Paciente
                          </Button>
          ...
          <TabsContent value="live" className="space-y-6">
            <Card>
              ...
              <CardContent>
                {surgeries
                  .filter((s) => s.schedule.status === "EN_CURSO")
                  .map((surgery) => (
                ...
                {surgeries.filter((s) => s.schedule.status === "EN_CURSO")
                  .length === 0 && (
                  <Alert>
